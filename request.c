...
void    checkRequest(int *clientSocket)
{
    ...
    createResponseMessage(responseMessage, request, hierarchy, version);
...
void    createResponseMessage(char* responseMessage, char* request, char* hierarchy, char* version)
{
    ...
    else
    {
        ...
        if (!strcmp(hierarchy, "/") || !strcmp(hierarchy, "/index.html") || !strcmp(hierarchy, "/connect.html"))
        {
            ...
            if (getHtmlFile(responseMessage, htmlFileBuffer, hierarchy))
